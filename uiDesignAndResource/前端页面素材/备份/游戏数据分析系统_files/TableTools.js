...
TableTools.prototype = {
	...
	"_fnGetDataTablesData": function ( oConfig )
	{
		...
		if ( oConfig.bHeader )
		{
			...
			/****************************************
			 * 解决多级表头保存问题
			 * @author gzhuangzhankun
			 * @date 2015-09-28
			 * start
			 ****************************************/
			if ( dt.aoHeader.length > 1) {
				var $dtScroll = $( this.dom.table ).closest( ".dataTables_scroll");
				var _ = this;
				var $trs = $dtScroll.find( ".dataTables_scrollHead .dataTables_scrollHeadInner table thead tr");
				var trOffsets = {};
				for(var i=0; i<=$trs.length; i++){
					trOffsets[i.toString()] = 0;
				}
				$trs.each(function(i, tr){
					var trOffset = trOffsets[i.toString()];
					for(var j=0; j<trOffset; j++){
						aRow.push( _._fnBoundData( _._fnHtmlDecode( "" ) , oConfig.sFieldBoundary, regex ) );
					}
					var itr = i;
					$(tr).find('th').each(function(){
						var colspanVal = parseInt($( this ).attr( "colspan"));
						var rowspanVal = parseInt($( this ).attr( "rowspan"));
						if(rowspanVal > 0){
							for(var j=1; j<rowspanVal; j++){
								var affected = (itr + j).toString();
								trOffsets[affected] += colspanVal;
							}
						}
						var title = $( this ).html().replace(/\n/g," ").replace( /<.*?>/g, "" ).replace(/^\s+|\s+$/g,"");
						aRow.push( _._fnBoundData( _._fnHtmlDecode( title ) , oConfig.sFieldBoundary, regex ) );
						if(colspanVal > 1){
...
TableTools.BUTTONS = {
	...
	"copy": $.extend( {}, TableTools.buttonBase, {
		...
		"fnComplete": function(nButton, oConfig, flash, text) {
			...
			// trick, important
			// 这里依赖于window.isEnglishVersion，这方法不好，但要添加options{isEnglishVersion:true;}比较麻烦
