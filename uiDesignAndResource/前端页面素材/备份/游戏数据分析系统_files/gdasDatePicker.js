/**
 *=======================================================================
 *GDAS日期控件，将MTA的控件包了一层，简化了对外调用接口
 *@author ：Zeyu Chen gzchenzeyu@corp.netease.com 陈泽裕
 *@version ： 2014-01-26
 *@modification list：2014-01-26  第一版
 *=======================================================================
 */
;
(function ($, window, document, undefined) {

    $.fn.gdasDatePicker = function (options) {

        var opts = $.extend({}, $.fn.gdasDatePicker.options, options);
        ...
        return this.each(function (index, input) {
            ...
            var member = {
                datePickerFrameId: "gdasDatePickerFrame_" + suffix,
                divDateId: "divDate_" + suffix,
                dateInputId: "dateInput_" + suffix,
                triggerId: "gdasDatePickerTrigger_" + suffix,
                datePickerTargetId: "gdasDatePickerTarget_" + suffix,
                datePickerId: "gdasDatePicker_" + suffix,
                aRecentDays: [0, 6, 13, 29, 89, 179],
                aRecentDaysCN: ['今天', '7天', '14天', '30天', '90天', '180天'],
                aRecentDaysEN: ['Today', '7Days', '14Days', '30Days', '90Days', '180Days'],
                aRecentDaysId: [],
                replaceBtn: "btn_compare_" + suffix
            };

            opts.inputTrigger = member.triggerId;
            opts.inputId = member.dateInputId;
            opts.target = member.datePickerTargetId;
            this.id = "gdasDatePickerHandler_" + suffix;
            opts.datePickerFrameId = member.datePickerFrameId;
            opts.divDateId = member.divDateId;
            opts.datePickerHandlerId = this.id;
            opts.replaceBtn = member.replaceBtn;
            opts.quickSelectId = "short_opt_" + suffix;
            opts.compareLabel = "compareLabel_" + suffix;


            //如果需要进行快速选择操作,则对快速跳转的href给定唯一id
            if (opts.addQuickSelect == true) {
                opts.aRecentDaysId = [];
                for(var i=0; i<member.aRecentDays.length; i++){
                    opts.aRecentDaysId.push('aRecent' + member.aRecentDays[i] + 'Days_' + suffix);
                }
            }

            addDatePickerQuickSelectHtml(input, member, opts);
            //如果给定value按逗号分割length >= 2, 则不是singleDay
            if (this.value.split(',').length >= 2) {
                opts.isSingleDay = false;
            ...
            //__method.str2date
            //如果需要比较日期，则初始化 startCompareDate and endCompareDate
            if (opts.needCompare == true) {
                var scDate = str2date(opts.startDate);
                var ecDate = str2date(opts.endDate);
                var delta = ecDate - scDate;
                //保证对比结束日期在开始日期的前面1天
                scDate.setUTCMilliseconds(scDate.getUTCMilliseconds() - delta - 24 * 60 * 60 * 1000);
                ecDate.setUTCMilliseconds(ecDate.getUTCMilliseconds() - delta - 24 * 60 * 60 * 1000);
                opts.startCompareDate = formatDate(date2ymd(scDate));
                opts.endCompareDate = formatDate(date2ymd(ecDate));
            }

            input._datePicker = new pickerDateRange(opts.inputId, opts);
    ...
    $.fn.gdasDatePicker.options = {
        //自动提交
        autoSubmit: false,
        needCompare: false,
        isSingleDay: false,
        addQuickSelect: true,
        isEnglishVersion: GDAS.isEnglishVersion,// added by ljjn1246 20150922, 自动检测语言环境
    ...
    function addDatePickerQuickSelectHtml(a, member, opts) {

        $(a).after('<div class="tool_date cf" id="' + member.datePickerFrameId + '"></div>');
        $('#' + member.datePickerFrameId).append('<button class="compareSwitchBtn" style="float:right" id="' + member.replaceBtn + '">' + (opts.isEnglishVersion ? 'Date Comparison' : '按时间对比') + '</button>');
        $('#' + member.datePickerFrameId).append('<div class="date" id="' + member.divDateId + '">' +
            '<span class="date_title" id="' + member.dateInputId + '"></span>' +
            '<a class="opt_sel" id="' + member.triggerId + '" href="#"><i class="i_orderd"></i></a></div>' +
            '<div id="' + member.datePickerTargetId + '"></div>'
        );
        if (opts.addQuickSelect) {
            var html = [];
            html.push('<div class="date-section cf" id="' + opts.quickSelectId + '">');
            html.push('<ul id="toolbar" class="select cf">');
            for(var i=0; i<member.aRecentDays.length; i++) {
                var id = opts.aRecentDaysId[i];
                var text = opts.isEnglishVersion ? member.aRecentDaysEN[i] : member.aRecentDaysCN[i];
                html.push('<li class=""><a id="' + id + '" href="javascript:void(0);" data=' + member.aRecentDays[i] + '>' + text + '</a></li>');
            }
            html.push('</ul></div>');
            $('#' + member.datePickerFrameId).append(html.join(''));
        }
    }
...
function pickerDateRange(inputId, options) {
    ...
    var defaults = {
        ...
        startDateId: 'targetStartDate', // 开始日期输入框ID
        startCompareDateId: 'targetStartCompareDate', // 对比开始日期输入框ID
        endDateId: 'targetEndDate', // 结束日期输入框ID
        endCompareDateId: 'targetEndCompareDate', // 对比结束日期输入框ID
        ...
        quickSelectId: 'short_opt',
        isEnglishVersion: GDAS.isEnglishVersion //added by ljjn1246 , 自动检测预言环境
    };
    //将对象赋给__method变量
    var __method = this;
    ...
    this.divDateId = this.mOpts.divDateId;

    this.compareTrigger = this.mOpts.compareTrigger + '_' + suffix;

    this.mOpts.compareTrigger = this.mOpts.compareTrigger + '_' + suffix;

    this.mOpts.startDateId = this.mOpts.startDateId + '_' + suffix;
    this.mOpts.endDateId = this.mOpts.endDateId + '_' + suffix;
    this.mOpts.compareCheckboxId = this.mOpts.compareCheckboxId + '_' + suffix;
    this.mOpts.startCompareDateId = this.mOpts.startCompareDateId + '_' + suffix;
    this.mOpts.endCompareDateId = this.mOpts.endCompareDateId + '_' + suffix;

    this.startDateId = 'startDate_' + suffix;
    this.endDateId = 'endDate_' + suffix;
    this.compareCheckboxId = 'needCompare_' + suffix;
    this.startCompareDateId = 'startCompareDate_' + suffix;
    this.endCompareDateId = 'endCompareDate_' + suffix;
    ...
    var wrapper = {
        ...
        ta: [
            ...
            '<input type="text" class="ta_ipt_text_s" name="' + this.startDateId + '" id="' + this.startDateId + '" value="' + this.mOpts.startDate + '"  />',
            '<span class="' + this.mOpts.joinLineId + '"> - </span>',
            '<input type="text" class="ta_ipt_text_s" name="' + this.endDateId + '" id="' + this.endDateId + '" value="' + this.mOpts.endDate + '"  /><br />',
            '</div>',
            '<div id="' + this.dateRangeCompareDiv + '">',
            '<input type="text" class="ta_ipt_text_s" name="' + this.startCompareDateId + '" id="' + this.startCompareDateId + '" value="' + this.mOpts.startCompareDate + '"  />',
            '<span class="' + this.mOpts.joinLineId + '"> - </span>',
            '<input type="text" class="ta_ipt_text_s" name="' + this.endCompareDateId + '" id="' + this.endCompareDateId + '" value="' + this.mOpts.endCompareDate + '"  />',
            '</div>',
            '</div>',
            '<div class="frm_btn">',
    ...
    var checkBoxWrapper = {
        ...
        ta: [
            ...
            '</label>',
            '<div class="date" id="' + this.compareInputDiv + '">',
    ...
    if (this.mOpts.theme == 'ta') {
        $(checkBoxWrapper[this.mOpts.theme].join('')).insertAfter($('#' + this.divDateId));
    } else {
        $(checkBoxWrapper[this.mOpts.theme].join('')).insertAfter($('#' + this.inputId));
    }
    if (this.mOpts.datePickerFrameId && '' != this.mOpts.datePickerFrameId){
        $('#' + this.mOpts.datePickerFrameId).attr('calendar-id', this.calendarId);
    }
    //根据传入参数决定是否展示日期输入框
    ...
    if (1 > $('#' + this.mOpts.startDateId).length) {
        $('' != this.mOpts.target ? '#' + this.mOpts.target : 'body').append('<input type="hidden" id="' + this.mOpts.startDateId + '" name="startDate" value="' + this.mOpts.startDate + '" />');
    } else {
        $('#' + this.mOpts.startDateId).val(this.mOpts.startDate);
    }
    if (1 > $('#' + this.mOpts.endDateId).length) {
        $('' != this.mOpts.target ? '#' + this.mOpts.target : 'body').append('<input type="hidden" id="' + this.mOpts.endDateId + '" name="endDate" value="' + this.mOpts.endDate + '" />');
    } else {
    ...
    if (false == this.mOpts.needCompare) {
        ...
        if (1 > $('#' + this.mOpts.startCompareDateId).length) {
            $('' != this.mOpts.target ? '#' + this.mOpts.target : 'body').append('<input type="hidden" id="' + this.mOpts.startCompareDateId + '" name="startCompareDate" value="' + this.mOpts.startCompareDate + '" />');
        } else {
            $('#' + this.mOpts.startCompareDateId).val(this.mOpts.startCompareDate);
        }
        if (1 > $('#' + this.mOpts.endCompareDateId).length) {
            $('' != this.mOpts.target ? '#' + this.mOpts.target : 'body').append('<input type="hidden" id="' + this.mOpts.endCompareDateId + '" name="endCompareDate" value="' + this.mOpts.endCompareDate + '" />');
        } else {
    ...
    $('#' + this.submitBtn).bind('click', function () {
        ...
        if (__method.mOpts.isSingleDay) {
            var newDate = $('#' + __method.mOpts.startDateId).val();
            $('#' + __method.mOpts.datePickerHandlerId).val(newDate);
        }
        else {
            var newDate = $('#' + __method.mOpts.startDateId).val() + ',' + $('#' + __method.mOpts.endDateId).val();
            $('#' + __method.mOpts.datePickerHandlerId).val(newDate);
        }
    ...
    $('#' + this.inputId).bind('click', function () {
        __method.mOpts.endDate = $('#' + __method.endDateId).val();
        __method.init();
    ...
    $('#' + this.mOpts.compareTrigger).bind('click', function () {
        __method.mOpts.endCompareDate = $('#' + __method.endCompareDateId).val();
        __method.init(true);
    ...
    if (this.mOpts.replaceBtn && $('#' + this.mOpts.replaceBtn).length > 0) {
        ...
        var replaceBtnId = this.mOpts.replaceBtn;
        ...
        $('#' + this.mOpts.replaceBtn).bind('click', function () {
            ...
            $('#' + __method.compareCheckboxId).attr('checked')
                ...
                $('#' + compareLabelId).hide();
                $('#' + quickSelectId).show();
                $(self).text(isEnglishVersion ? 'Date Comparison' : '按时间对比');
            }()
                : function () {
                $('#' + __method.compareCheckboxId).attr('checked', 'checked');
                $('#' + compareLabelId).show();
...
pickerDateRange.prototype.init = function (isCompare) {
    ...
    var endDateMoment;
    if(isCompare){
        endDateMoment = '' == this.mOpts.endCompareDate ? moment().tz(this.mOpts.timezone) : moment.tz(this.mOpts.endCompareDate, this.mOpts.timezone);
    }else{
        endDateMoment = '' == this.mOpts.endDate ? moment().tz(this.mOpts.timezone) : moment.tz(this.mOpts.endDate, this.mOpts.timezone);
    }
    // 日历结束时间
    this.calendar_endDate = endDateMoment.clone();
    this.calendar_endDate.month(endDateMoment.month() + 1).date(0);

    // 计算并显示以 endDate 为结尾的最近几个月的日期列表
    for (var i = 0; i < this.mOpts.calendars; i++) {
        var td = null;
        if (this.mOpts.theme == 'ta') {
            td = this.fillDate(endDateMoment.year(), endDateMoment.month(), i);
        }
        else {
            td = document.createElement('td');
            $(td).append(this.fillDate(endDateMoment.year(), endDateMoment.month(), i));
        ...
        endDateMoment.month(endDateMoment.month() - 1);
    }

    // 上一个月
    $('#' + this.preMonth).bind('click', function () {
        __method.calendar_endDate.date(0);
        if(isCompare){
            __method.mOpts.endCompareDate = __method.calendar_endDate.format('YYYY-MM-DD')
            __method.init(isCompare);
            //如果是单月选择的时候，要控制input输入框 added by johnnyzheng 2011-12-19
            if (1 == __method.mOpts.calendars) {
                if ('' == $('#' + __method.startCompareDateId).val()) {
                    __method.changeInput(__method.startCompareDateId);
                }
                else {
                    __method.changeInput(__method.endCompareDateId);
                }
            }
        }else{
            __method.mOpts.endDate = __method.calendar_endDate.format('YYYY-MM-DD');
            __method.init(isCompare);
            //如果是单月选择的时候，要控制input输入框 added by johnnyzheng 2011-12-19
            if (1 == __method.mOpts.calendars) {
    ...
    $('#' + this.nextMonth).bind('click', function () {
        __method.calendar_endDate.month(__method.calendar_endDate.month() + 2).date(0);
        if(isCompare){
            __method.mOpts.endCompareDate = __method.calendar_endDate.format('YYYY-MM-DD');
            __method.init(isCompare);
            //如果是单月选择的时候，要控制input输入框 added by johnnyzheng 2011-12-19
    ...
    // 日历开始时间
    if(endDateMoment.month() == 11){
        this.calendar_startDate = moment.tz([endDateMoment.year() + 1, 0, 1], this.mOpts.timezone);
    }else{
        this.calendar_startDate = moment.tz([endDateMoment.year(), endDateMoment.month() +1, 1], this.mOpts.timezone);
    }
    // 初始化时间选区背景
    ...
    if(__method.mOpts.addQuickSelect){
        for(var i=0; i<__method.mOpts.aRecentDaysId.length; i++){
            var quickId = __method.mOpts.aRecentDaysId[i];
            if ($('#' + quickId).length > 0) {
                $('#' + quickId).off('click').on('click', function () {
                    var period = parseFloat($(this).attr('data'));
                    ...
                    //拼接提交时间串
                    var timeObj = __method.getSpecialPeriod(period);
                    $('#' + __method.startDateId).val(timeObj.otherday);
                    $('#' + __method.endDateId).val(timeObj.today);
                    ...
                    if ($('#' + __method.mOpts.replaceBtn).length > 0) {
                        $('.contrast').hide();
                        $('#' + __method.mOpts.replaceBtn).text(__method.mOpts.isEnglishVersion ? 'Date Comparison' : '按时间对比');
                    }
                    //点击提交
...
pickerDateRange.prototype.getSpecialPeriod = function (period) {
    ...
    (true == __method.mOpts.isTodayValid && ('' != __method.mOpts.isTodayValid) || 2 > period) ? '' : now.date(now.date() - 1);
    var todayStr = now.format('YYYY-MM-DD');
    var back = now.unix() - period * 24 * 60 * 60;
    if(back < __method.mOpts.minValidDate){
        back = __method.mOpts.minValidDate;
    }
    now = moment.unix(back).tz(__method.mOpts.timezone);
    var otherdayStr = now.format('YYYY-MM-DD');
...
pickerDateRange.prototype.removeCSS = function (isCompare, specialClass) {
    ...
    // 整个日期列表的开始日期
    var bDate = this.calendar_startDate.clone();
    var cla = '';
    // 从开始日期循环到结束日期
    for (var d = bDate; d.unix() <= this.calendar_endDate.unix(); d.date(d.date() + 1)) {
        if (0 == isCompare) {
            // 移除日期样式
        ...
        // 移除指定样式
        $('#' + this.calendarId + '_' + d.format('YYYY-MM-DD')).removeClass(cla);
        $('#' + this.calendarId + '_' + d.format('YYYY-MM-DD')).removeClass(this.mOpts.firstCss).removeClass(this.mOpts.lastCss).removeClass(this.mOpts.clickCss);
...
pickerDateRange.prototype.addCSS = function (isCompare, specialClass) {
    ...
    // 获取4个日期
    var startDate = moment($('#' + this.startDateId).val());
    var endDate = moment($('#' + this.endDateId).val());
    var startCompareDate = moment($('#' + this.startCompareDateId).val());
    var endCompareDate = moment($('#' + this.endCompareDateId).val());
    ...
    var cla = '';
    for (var d = sDate.clone(); d.unix() <= eDate.unix(); d.date(d.date() + 1)) {
        if (0 == isCompare) {
            // 添加日期样式
            cla = this.mOpts.theme + '_' + this.mOpts.selectCss;
            $('#' + this.calendarId + '_' + d.format('YYYY-MM-DD')).removeClass(this.mOpts.firstCss).removeClass(this.mOpts.lastCss).removeClass(this.mOpts.clickCss);
        ...
        $('#' + this.calendarId + '_' + d.format('YYYY-MM-DD')).attr('class', cla);
    }
    if (this.mOpts.theme == 'ta') {
        //为开始结束添加特殊样式
        $('#' + this.calendarId + '_' + sDate.format('YYYY-MM-DD')).removeClass().addClass(this.mOpts.firstCss);
        $('#' + this.calendarId + '_' + eDate.format('YYYY-MM-DD')).removeClass().addClass(this.mOpts.lastCss);
        //如果开始结束时间相同
        sDate.unix() == eDate.unix() && $('#' + this.calendarId + '_' + eDate.format('YYYY-MM-DD')).removeClass().addClass(this.mOpts.clickCss);
    }
};
...
pickerDateRange.prototype.selectDate = function (ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        // 更改对应输入框的值
        $('#' + this.dateInput).val(ymd);
        $('#' + this.endDateId).val(ymd);
...
//                    'endDate': $('#' + this.mOpts.startDateId).val(),
...
//                });
        ...
        if (-1 == this.compareStrDate(ymd, $('#' + this.startDateId).val())) {
            ...
            // 更改对应输入框的值(开始时间)
            $('#' + this.startDateId).val(ymd);
            ymd = $('#' + this.dateInput).val();
        }
        // 更改对应输入框的值
        $('#' + this.dateInput).val(ymd);
        // 切换输入框焦点
...
//                });
        ...
        if (-1 == this.compareStrDate(ymd, $('#' + this.startCompareDateId).val())) {
            ...
            // 更改对应输入框的值(开始时间)
            $('#' + this.startCompareDateId).val(ymd);
            ymd = $('#' + this.dateInput).val();
        }
        // 更改对应输入框的值
        $('#' + this.dateInput).val(ymd);
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    if (0 < clientWidth && $("#" + this.calendarId).width() + pos.left+2>= clientWidth) {//modified by ljjn1246 20150923, 宽度+左偏移量+2（边框像素）达到了屏幕宽度则居右
        left = pos.left + $('#' + this.inputId).closest('div').width() - $("#" + this.calendarId).width() + ((/msie/i.test(navigator.userAgent) && !(/opera/i.test(navigator.userAgent))) ? 5 : 0);
        __method.mOpts.theme == 'ta' && (left += 0);
    }
    $("#" + this.calendarId).css('left', left + 'px');
    //$("#" + this.calendarId).css('top', pos.top + (offsetHeight ? offsetHeight- 1 : (__method.mOpts.the me=='ta'?35:22)) + 'px');
    $("#" + this.calendarId).css('top', pos.top + (__method.mOpts.theme == 'ta' ? 35 : 22) + 'px');//TODO
    //第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.close = function (btnSubmit) {
    ...
    if (btnSubmit) {
        ...
        if (this.mOpts.shortOpr === true) {
            ...
            //code below add by ZeyuChen
            if (this.mOpts.isSingleDay == true) {
                $('#' + this.inputId).val($('#' + this.startDateId).val());
            }
            else {
                $('#' + this.inputId).val($('#' + this.startDateId).val() + ('' == $('#' + this.endDateId).val() ? '' : (this.mOpts.isEnglishVersion ? " to " : this.mOpts.defaultText) + $('#' + this.endDateId).val()));
            }

        }
        //判断当前天是否可选，来决定从后往前推修改日期是从哪一点开始
        var now = moment().tz(this.mOpts.timezone);
        var nDateTime = (true == this.mOpts.isTodayValid) ? now.unix(): now.unix()- (24 * 60 * 60);
        if(this.mOpts.isSingleDay == true){
            $('#' + this.endDateId).val($('#' + this.startDateId).val());
        }

        var bDateTime = moment.tz($('#' + this.startDateId).val(), this.mOpts.timezone).unix();
        var eDateTime = moment.tz($('#' + this.startDateId).val(), this.mOpts.timezone).unix();
        ...
        var _val = this.mOpts.shortOpr == true ? $('#' + this.startDateId).val() : ($('#' + this.startDateId).val() + ('' == $('#' + this.endDateId).val() ? '' : (this.mOpts.isEnglishVersion ? " to " : this.mOpts.defaultText) + $('#' + this.endDateId).val()));
        //code below added by Zeyu Chen
        if (this.mOpts.isSingleDay) {
            _val = $('#' + this.startDateId).val();
        }
        // 把开始、结束时间显示到输入框 (PS:如果选择的今日，昨日，则只填入一个日期)
        ...
        if (this.mOpts.theme != 'ta') {
            if ('' != $('#' + this.startCompareDateId).val() && '' != $('#' + this.endCompareDateId).val()) {
                var bcDateTime = moment.tz($('#' + this.startCompareDateId).val(), this.mOpts.timezone).unix();
                ...
                if (_ecDateTime > nDateTime) {
                    ...
                    $('#' + this.startCompareDateId).val(moment.unix(_ecDateTime + bDateTime - eDateTime).tz(this.mOpts.timezone).format('YYYY-MM-DD'));
                }
                $('#' + this.endCompareDateId).val(moment.unix(_ecDateTime).tz(this.mOpts.timezone).format('YYYY-MM-DD'));

                //把开始结束对比时间大小重新矫正一下
                var bcDateTime = moment.tz($('#' + this.startCompareDateId).val(), this.mOpts.timezone).unix();
                var ecDateTime = moment.tz($('#' + this.endCompareDateId).val(), this.mOpts.timezone).unix();
        ...
        var _compareVal = this.mOpts.shortOpr == true ? $('#' + this.startCompareDateId).val() : ($('#' + this.startCompareDateId).val() + ('' == $('#' + this.endCompareDateId).val() ? '' : (this.mOpts.isEnglishVersion ? " to " : this.mOpts.defaultText) + $('#' + this.endCompareDateId).val()));
        if (this.mOpts.isSingleDay) {
            _compareVal = $('#' + this.startCompareDateId).val();
        }
        if (input && input.tagName == 'INPUT') {
        ...
        // 计算相隔天数
        var step = (bDateTime - eDateTime) / 86400;


        //modified by dorsywang
        //这里增加选中后高亮快捷天数选择器
        var stepDay = Math.abs(step) + 1;
        var fastDayId = 'aRecent' + stepDay + 'Days';

        if (this.mOpts[fastDayId]) {
            $("#" + this.mOpts[fastDayId]).parent().addClass('active').siblings().removeClass('active');
        } else {
            $("#" + this.mOpts['aRecent7Days']).parent().removeClass('active').siblings().removeClass('active');
        }
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
    ...
    // 当月第一天
    var firstDayOfMonth = moment.tz([year, month, 1], __method.mOpts.timezone);
    var dateBegin = firstDayOfMonth.clone();
    dateBegin.date(1 - dateBegin.day());

    // 当月最后一天
    var lastDayOfMonth;
    if(month == 11){
        lastDayOfMonth = moment.tz([year+1, 0], __method.mOpts.timezone).date(0);
    }else{
        lastDayOfMonth = moment.tz([year, month + 1], __method.mOpts.timezone).date(0);
    }
    var dateEnd = lastDayOfMonth.clone();
    dateEnd.date(dateEnd.date() + 6 - dateEnd.day());

    var now = moment().tz(__method.mOpts.timezone);
    var dToday = now.date();
    var mToday = now.month();
    var yToday = now.year();
    ...
    if (isTaTheme) {
        ...
        var days = this.mOpts.isEnglishVersion ? ['S', 'M', 'T', 'W', 'T', 'F', 'S'] : ['日', '一', '二', '三', '四', '五', '六'];
        for (var i = 0; i < 7; i++) {
    ...
    for (var d = dateBegin; d.unix() <= dateEnd.unix(); d.date(d.date() + 1)) {
        if (d.unix() < firstDayOfMonth.unix()) { // 当前月之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '-1';
        } else if (d.unix() > lastDayOfMonth.unix()) { // 当前月之后的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '1';
        } else if ((this.mOpts.stopToday == true && d.unix() > now.unix()) || d.unix() < __method.mOpts.minValidDate || ('' !== __method.mOpts.maxValidDate && d.unix() > __method.mOpts.maxValidDate)) { // 当前时间之后的日期，或者开启统计之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期
            deviation = '0';
            if (d.date() == dToday && d.month() == mToday && d.year() == yToday) {
            ...
            //让周末不可选不可选
            if (this.mOpts.weekendDis && (d.day() == 6 || d.day() == 0)) {
                tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
                deviation = '3';
            }
            //让周几不可选
            if (this.mOpts.disCertainDay && this.mOpts.disCertainDay.length > 0) {
                for (var p in this.mOpts.disCertainDay) {
                    if (!isNaN(this.mOpts.disCertainDay[p]) && d.day() === this.mOpts.disCertainDay[p]) {
                        tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
                        deviation = '4';
                    }
                }
                ...
                for (var p in this.mOpts.disCertainDate) {
                    if (!isNaN(this.mOpts.disCertainDate[p]) || isNaN(parseInt(this.mOpts.disCertainDate[p]))) {
                        if (this.mOpts.disCertainDate[0] === true) {
                            isDisabled = !!(d.date() !== this.mOpts.disCertainDate[p]);
                            if (!isDisabled) {
                                break;
                            }
                        } else {
